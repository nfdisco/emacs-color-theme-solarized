    ...
    (let ((bg-back   '(:background back))
      ...
      (mapcar (lambda (face) (apply 'create-face-spec face))
              `(;; basic
                ...
                (org-mode-line-clock-overrun (:inherit mode-line ,@fg-red))
                ...
                (magit-bisect-skip (,@fg-orange))
                (magit-blame-dimmed (:inherit shadow))
                (magit-blame-highlight (,@bg-base02))
                (magit-branch-current (,@fg-blue ,@fmt-undb))
                (magit-branch-local (,@fg-blue ,@fmt-bold))
                (magit-branch-remote (,@fg-green ,@fmt-bold))
                (magit-branch-remote-head (,@fg-green ,@fmt-undb))
                (magit-cherry-equivalent (,@fg-magenta))
                (magit-branch-unmatched (,@fg-cyan))
                (magit-diff-added (,@fg-green))
                (magit-diff-added-highlight (,@fg-green ,@bg-base02))
                (magit-diff-base (,@fg-yellow))
                (magit-diff-base-highlight (,@fg-yellow ,@bg-base02))
                (magit-diff-conflict-heading (,@bg-base02))
                (magit-diff-context (:inherit default))
                (magit-diff-context-highlight (:inherit shadow ,@bg-base02))
                (magit-diff-file-heading (,@fmt-bold))
                (magit-diff-file-heading-highlight (,@fmt-bold ,@bg-base02))
                (magit-diff-file-heading-selection (,@fmt-bold ,@bg-base03))
                (magit-diff-hunk-heading (:inherit magenta))
                (magit-diff-hunk-heading-highlight
                 (:inherit magit-diff-hunk-heading ,@bg-base02))
                (magit-diff-hunk-heading-selection
                 (:inherit magit-diff-hunk-heading ,@fmt-bold ,@bg-base03))
                (magit-diff-removed (,@fg-red))
                (magit-diff-removed-highlight (,@fg-red ,@bg-base02))
                (magit-diffstat-added (,@fg-green))
                (magit-diffstat-removed (,@fg-red))
                (magit-dimmed (:inherit shadow))
                (magit-filename (,@fg-blue))
                (magit-hash (:inherit shadow))
                (magit-head (,@fg-violet))
                (magit-header-line (,@fmt-bold ,@fg-yellow))
                (magit-header-line-key (,@fg-green))
                (magit-header-line-log-select (,@fmt-bold))
                (magit-log-author (,@fg-cyan))
                (magit-log-date (,@fg-orange))
                (magit-log-graph (,@fg-orange))
                ...
                (magit-refname (,@fg-orange))
                (magit-section-heading (,@fmt-bold ,@fg-yellow))
                (magit-section-heading-selection (,@fg-blue))
                (magit-section-highlight (,@bg-base02))
                (magit-section-secondary-heading (,@fmt-bold))
                (magit-sequence-done (:inherit shadow))
                (magit-sequence-drop (,@fg-red))
                (magit-sequence-exec (:inherit shadow))
                (magit-sequence-head (,@fg-blue))
                (magit-sequence-onto (:inherit shadow))
                (magit-sequence-part (,@fg-yellow))
                (magit-sequence-pick (,@fg-blue))
                (magit-sequence-stop (,@fg-green))
                (magit-signature-bad (,@fg-red ,@fmt-bold))
                (magit-signature-error (,@fg-red))
                (magit-signature-expired (,@fg-orange))
                (magit-signature-expired-key (,@fg-orange))
                (magit-signature-good (,@fg-green))
                (magit-signature-revoked (,@fg-magenta))
                (magit-signature-untrusted (,@fg-blue))
